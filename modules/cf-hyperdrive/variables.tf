...
variable "origin_database_url" {
  description = "Full PostgreSQL connection URL for the origin database. It accepts postgresql://<user>:<password>@<host>:<port>/<database> or postgres://<user>:<password>@<host>:<port>/<database>; user and database cannot contain : @ / ? # %, password cannot contain @ ? # %, host cannot contain : @ / ? # % [ ], and port has 1 to 5 digits."
  type        = string
  sensitive   = true
