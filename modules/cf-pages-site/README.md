...
The CNAME always has `type = "CNAME"`, `ttl = 1`, and `proxied = true`. When `service_bindings` is non-empty, the project sets the same service map for both production and preview deployments; each binding's `service` names a Worker script in the same account, which the module does not create.
