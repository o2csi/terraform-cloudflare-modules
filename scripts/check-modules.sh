...
# Assertions 5 and 6 run tofu init and tofu validate, and assertion 5 also runs tests when present; these download and start the providers the
# checkout names (the ten modules constrained ~> 5.0 resolve the newest 5.x release); run this on a checkout you would run tofu on.
set -euo pipefail
...
for module_dir in "${module_dirs[@]}"; do
  ...
  if [[ -d "${staged_module}/tests" ]] && ! tofu -chdir="${staged_module}" test -no-color; then
