...
- What it reads: the checker itself runs from your worktree while the content it judges comes from an export of the Git index (`git add` what you want checked); untracked and unstaged paths are not exported; Git configuration and `info/attributes` still apply.
...
- What it refuses before running tofu, as a fixed lexical profile rather than an HCL parser; early refusals include: a module directory whose name is not lowercase `[a-z0-9-]`, any symlink under `modules/`, `/*` and `<<` anywhere in `variables.tf`/`outputs.tf`, `/*` outside strings in any other module-root `.tf`, `/*` and `<<` outside strings or a `}` followed by text in an extracted README example or a discovered test file (dot-prefixed test files are ignored, as `tofu test` ignores them), a `module`/`run` header that is not canonical, that arrives while the previous block is open or that is still open at end of file, a `run` block without `command = plan`, a JSON test file, two test files sharing a stem, a test file outside the module root and `tests/`, a module README without exactly one complete `hcl` fence holding exactly one module block whose `source` is this repository's URL naming its own module, or a `?ref=` that is not a literal ref. The header of `scripts/check-modules.sh` describes the profile in detail; the script is authoritative.
- What it runs: `tofu fmt -check`, then per module `init`, `validate`, `test`, then each module README's single column-zero ```` ```hcl ```` example with its source pointed at the local copy, through `init` and `validate`; tofu receives only `PATH`, an empty `HOME`, a private `TMPDIR`, and the plugin cache; there is no provider lock file, so each `init` takes the newest provider matching `~> 5.0`; this is not a sandbox: run it only on a branch you would run a script from.
- What it needs and where it runs: Git 2.40 or later, OpenTofu, Bash, GNU awk, coreutils, findutils, grep and sed, and network access to the registry; locally before pushing, and the workflow is configured for pushes to `main` and `pull_request` events targeting `main`; GitHub does not run it on a pull request with merge conflicts or while an external contributor's run awaits approval ([`.github/workflows/check-modules.yml`](.github/workflows/check-modules.yml); on a pull request the check judges the prospective merge).
