...
- **Checks** — `scripts/check-modules.sh` judges an export of the Git index (stage what you want checked with `git add`): the layout, the interface descriptions, every module's `tofu validate` and `tofu test`, and every README example, with tofu inheriting none of your environment; it is not a sandbox, so run it only on a branch you would run a script from. Run it before pushing.
