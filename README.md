...
- What it refuses before running tofu, as a fixed lexical profile rather than an HCL parser: a module directory whose name is not lowercase `[a-z0-9-]`, any symlink under `modules/`, `/*` and `<<` anywhere in `variables.tf`/`outputs.tf`, `/*` outside strings in any other module-root `.tf`, `/*` and `<<` outside strings in README examples and test files, a `}` followed by text, a `module`/`run` header that is not canonical or still open at end of file, a test file outside the module root and `tests/`, a README example whose `source` is not this repository's URL naming its own module, or a `?ref=` that is not a literal ref. The header of `scripts/check-modules.sh` describes the profile in detail; the script is authoritative.
